...
<div class="container">
    <h2 class="text-center"><img src="img/selecione_questionario.png" class="img" width="500" height="100"></h2>
        <div class="row">
            <?php
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $questionario_id = $row['id_questionario'];
                    $questionario_nome = $row['nome'];
                    $cor_atual = $cores[$cor_index % count($cores)];

                    echo '<div class="col-md-4">';
                    echo '<div class="module-box" style="background-color: ' . $cor_atual . ';">';
                    echo '<h4><strong style="font-size: 20px;">' . $questionario_nome . '</strong></h4>';
                    echo '<a href="user_questionario.php?id_questionario=' . $questionario_id . '" class="btn btn-primary btn-branco">Responder</a>';
                    echo '</div>';
                    echo '</div>';
                    $cor_index++;
